...
import { resolveLanguage, siteLanguageOptions, useLanguage } from "../lib/i18n";
...
export function LanguagePrompt() {
  ...
  const browserLabel = useMemo(() => languageLabel(resolveLanguage(browserLanguage)), [browserLanguage, languageLabel]);

  useEffect(() => {
    const saved = window.localStorage.getItem(languageStorageKey);
    const detected = navigator.languages?.[0] || navigator.language || "en";
    setBrowserLanguage(detected);
    setSelectedLanguage(resolveLanguage(saved || detected));
    setSavedLanguage(saved ? resolveLanguage(saved) : null);
